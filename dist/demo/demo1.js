...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "dc0bf4dcce3ad78a3b2a";
...
const MENU_ITEMS_DEFAULT = {
  insertColumnRight: {
    ...
    handler() {
      ...
      let colIndex = getColToolCellIndexByBoundary(this.columnToolCells, this.boundary, (cellRect, boundary) => {
        return Math.abs(cellRect.x + cellRect.width - boundary.x1) <= contants_ERROR_LIMIT;
  ...
  insertColumnLeft: {
    ...
    handler() {
      const tableContainer = external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.find(this.table);
      let colIndex = getColToolCellIndexByBoundary(this.columnToolCells, this.boundary, (cellRect, boundary) => {
        return Math.abs(cellRect.x - boundary.x) <= contants_ERROR_LIMIT;
      }, this.quill.root.parentNode);
  ...
  mergeCells: {
    ...
    handler() {
      ...
      const rowspan = tableContainer.rows().reduce((sum, row) => {
        ...
        if (rowRect.y > this.boundary.y - contants_ERROR_LIMIT && rowRect.y + rowRect.height < this.boundary.y + this.boundary.height + contants_ERROR_LIMIT) {
          sum += 1;
        }
        return sum;
      }, 0);
      ...
      const colspan = this.columnToolCells.reduce((sum, cell) => {
        ...
        if (cellRect.x > this.boundary.x - contants_ERROR_LIMIT && cellRect.x + cellRect.width < this.boundary.x + this.boundary.width + contants_ERROR_LIMIT) {
          sum += 1;
        }
        return sum;
      }, 0);
  ...
  deleteColumn: {
    ...
    handler() {
      ...
      let colIndexes = getColToolCellIndexesByBoundary(this.columnToolCells, this.boundary, (cellRect, boundary) => {
        return cellRect.x + contants_ERROR_LIMIT > boundary.x && cellRect.x + cellRect.width - contants_ERROR_LIMIT < boundary.x1;
      }, this.quill.root.parentNode);
...
// CONCATENATED MODULE: ./src/quill-better-table-plus.js
...
Quill.register({
  'modules/better-table': src_quill_better_table_plus_js__WEBPACK_IMPORTED_MODULE_0__["default"]
}, true);
window.onload = () => {
